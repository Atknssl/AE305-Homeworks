...
even with a larger step size. In our case, error became significant only at the endpoints. That error is happened because
algorithm jumps to the next time step even if the true result is closer to the current evaluation point than the time step size.
...
\begin{figure*}[htp]
        \centering
        \subfigure[$\Delta t=1$.]{\includegraphics[max height=6cm]{graphs/compare1.eps}}\hspace\fill
        \subfigure[$\Delta t=3$.]{\includegraphics[max height=6cm]{graphs/compare3.eps}}
